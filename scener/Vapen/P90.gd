...
var _ljud : Array = [preload("res://scener/ljud/P90_skott.tscn"), preload("res://scener/ljud/silenced_pistol_skott.tscn"), preload("res://scener/ljud/empty_skott.tscn")]
var skott_ljud
...
func _process(delta):
	...
	if ammo == 50 and ammo >= 40:
		$Sprite2D.frame = 0
	if ammo <= 40 and ammo >= 30:
		$Sprite2D.frame = 1
	if ammo <= 30 and ammo >= 25:
		$Sprite2D.frame = 2
	if ammo <= 20 and ammo >= 10:
		$Sprite2D.frame = 3
...
func _on_p_90_anims_animation_finished(anim_name):
	if anim_name == "recoil":
		can_shoot = true
		get_parent().shooting = false
